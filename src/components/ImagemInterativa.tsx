...
function ImagemInterativa( { principalSrc, principalAlt, secundarioSrc, secundarioAlt }: ImagemInterativaProps) {
    return (
        ...
        >
            ...
            alt={principalAlt} 
            />
            <img className="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-500 ease-in-out group-hover:opacity-100 group-focus:opacity-100"
            src= {secundarioSrc}
            alt= {secundarioAlt}
            />
        </div>
